...
using MyYamlParser;
using IMyNoSqlNodePersistenceSettings = MyNoSqlServer.Domains.IMyNoSqlNodePersistenceSettings;
...
namespace MyNoSqlServer.Api
{
    ...
    public static class SettingsLoader
    {
        public static SettingsModel LoadSettings()
        {
            ...
            {
                homeFolder = Environment.GetEnvironmentVariable("HOMEDRIVE") +
            ...
            if (string.IsNullOrEmpty(settingsModel.PersistenceDest))
            {
                ...
                Console.WriteLine(
                    "In case to enable, please specify 'BackupAzureConnectString' in env variable or in json file: ~/.mynosqlserver");
                throw new Exception("PersistencePath should not be Empty");
            }

            if (!settingsModel.PersistenceDest.StartsWith("http") &&
                !settingsModel.PersistenceDest.StartsWith("DefaultEndpointsProtocol"))
            {
                throw new Exception(
